import 'package:flutter/material.dart';
import 'package:flutter_hooks/flutter_hooks.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';
import 'package:stamp_rally_v2_fvm/core/router/router.dart';
import 'package:stamp_rally_v2_fvm/feature/home/home.dart';
import 'package:stamp_rally_v2_fvm/feature/stamp_list/stamp_list.dart';

class StampRallyEventDetailPage extends HookConsumerWidget {
  const StampRallyEventDetailPage({super.key, required this.url});

  final String url;

  // 詳細ページをスタック
  static void push(BuildContext context, String url) {
    StampRallyEventDetailPageRoute(url).push(context);
  }

  // 全てをページを置き換え
  static void go(BuildContext context, String url) {
    StampRallyEventDetailPageRoute(url).go(context);
  }

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final currentIndex = useState(0);
    final pageList = [StampListPage(url: url), const HomePage()];
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(title: const Text("イベント詳細")),
        // appBar: AppBar(),
        body: pageList[currentIndex.value],
        bottomNavigationBar: BottomNavigationBar(
          currentIndex: currentIndex.value,
          items: const [
            BottomNavigationBarItem(
              icon: Icon(Icons.list),
              label: 'スタンプラリー一覧',
            ),
            BottomNavigationBarItem(
              icon: Icon(Icons.summarize),
              label: '概要',
            ),
          ],
          onTap: (index) {
            currentIndex.value = index;
          },
        ),
      ),
    );
  }
}
